 ...
 */
@Slf4j
public class Es7Dumper implements IBusiSender<DumpEvent> {
	...
	private volatile static Map<String, Rule> ruleMap = new HashMap<String, Rule>();
	...
	public synchronized void initParams(JSONObject params) {
		...
		for (Rule rule : rulesList) {
			...
			Map<String, Propertie> queryMapping_tc_all = ESClientOnlyOne.getInst().getESClient()
					.queryMapping_tc_all(rule.getItems().get(RuleItem.index));
			...
			if (hasprimarys) {
				...
				if (isroot) {// 根元素或是没有关联关联的索引
					...
					if (relaObjMap.get(curRule) != null && !relaObjMap.get(curRule).isEmpty()) {
						String tableNameTrue = (tablename.length() >= curRule.getTbLength())
								? tablename.substring(0, curRule.getTbLength())
								: tablename;
