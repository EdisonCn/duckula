...
public class KafkaConsumer<T> implements IConsumer<byte[]> {
	...
	protected volatile static Map<String, String[]> primarysMap = new HashMap<>();
	protected volatile static Map<String, String[]> colsMap = new HashMap<>();
	...
	private IConsumerSender<T> loadPluginSender(SenderConsumerEnum pluginSender, JSONObject senderParams) {
		...
		Plugin plugin = PluginAssit.newPlugin(
				IOUtil.mergeFolderAndFilePath(rootDir.getPath(), pluginSender.getPluginJar()),
		...
		try {
			...
			if (duckulaEvent.getOptType() == OptType.delete) {
				...
				if (isIde || YesOrNo.yes == this.consumer.getNeedAllCol()) {
					if (rs.next()) {
						for (String colName : colsMap.get(keymapkey)) {// TODO 会报空指针？？
							try {
	...
	private Connection getConn(String addProp) throws SQLException {
		...
		if (StringUtil.isNotNull(addProp)) {
			...
			// JSONObject.toJavaObject(ZkClient.getInst().getZkData(ZkPath.dbinsts.getPath(consumer.getDrdsDbInst())),
			// DbInstance.class);
			...
			if (!DruidAssit.getDataSourceMap().containsKey(sourceKey)) {
				DbInstance dbInstance = JSONObject
						.toJavaObject(ZkClient.getInst().getZkData(ZkPath.dbinsts.getPath(addProp)), DbInstance.class);
				Properties props = new Properties();
				props.put("host", dbInstance.getUrl());
				props.put("username", dbInstance.getUser());
				props.put("password", dbInstance.getPwd());
				props.put("port", dbInstance.getPort());
				props.put("urlparam",
						"zeroDateTimeBehavior=convertToNull&useUnicode=true&characterEncoding=UTF-8&autoReconnect=true&failOverReadOnly=false");
