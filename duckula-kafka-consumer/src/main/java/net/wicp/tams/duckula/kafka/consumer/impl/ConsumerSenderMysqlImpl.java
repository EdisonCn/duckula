...
public class ConsumerSenderMysqlImpl extends ConsumerSenderAbs<Builder> {
    ...
    public ConsumerSenderMysqlImpl(JSONObject params) {
        super(params);
    }
    ...
    public Builder packObj(DuckulaEvent duckulaEvent, Map<String, String> datamap, Rule rule, String[] primaries) {
        if (dsmap.get(rule) == null) {
            synchronized (ConsumerSenderMysqlImpl.class) {
                if (dsmap.get(rule) == null) {
...
//        String[] keys = primaryMap.get(String.format("%s.%s", duckulaEvent.getDb(), duckulaEvent.getTb()));
        newBuilder.addAllKeys(Arrays.asList(primaries));
